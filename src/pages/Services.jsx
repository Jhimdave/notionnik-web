...
/* ── Cache config ──────────────────────────────────────────────────
   ...
   TTL  : 15 minutes
   VER  : bump this string any time the data shape changes — it
          forces all clients to drop the old cache immediately.
   ─────────────────────────────────────────────────────────────── */
const CACHE_KEY = "services_cache_v2";   // ← bumped from v1 (clears old cache)
...
function parseService(page) {
  ...
  // Features — multi-select (add a "Features" column in Notion if you want)
  const features = props.Features?.multi_select?.map(f => f.name) || []

  // Tagline — plain text (add a "Tagline" column in Notion if you want)
  const tagline = props['Service Header']?.rich_text?.map(r => r.plain_text).join('') || ''

  return { title, desc, icon, logo, id: page.id, tools, features, tagline }
}
...
function ServiceModal({ service, onClose }) {
  ...
  // staticData is the local fallback — Notion data takes priority
  const staticData = SERVICES_DATA.find(s =>
    s.title.toLowerCase().includes(service.title.toLowerCase().split(' ')[0])
  )

  // Resolve each field: Notion first, static fallback second
  const description = service.desc || staticData?.long || 'Professional service tailored to your workflow.'
  const tagline     = service.tagline || staticData?.tagline || ''
  const tools       = service.tools?.length > 0 ? service.tools : staticData?.tools || []
  const features    = service.features?.length > 0 ? service.features : staticData?.features || []
    ...
    >
      ...
      <div className="relative w-full max-w-xl max-h-[85vh] flex flex-col bg-navy-900 border border-brand-500/25 rounded-3xl shadow-brand-lg overflow-hidden">
        ...
        <div className="flex items-start justify-between p-7 pb-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {tagline && <p className="text-brand-400/70 text-xs font-mono mt-0.5">{tagline}</p>}
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto p-7 space-y-6">
          ...
          {/* Description — Notion first */}
          <p className="text-blue-100/75 leading-relaxed">{description}</p>

          {/* Features — Notion multi-select OR staticData fallback */}
          {features.length > 0 && (
            <div>
                  ...
                  <div key={f} className="flex items-center gap-2.5 bg-navy-800/50 rounded-xl px-3.5 py-2.5">
                    <div className="w-4 h-4 rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center flex-shrink-0">
                      <svg width="7" height="7" viewBox="0 0 12 10" fill="none">
                        <path d="M1 5l3.5 3.5L11 1" stroke="#2d8ef5" strokeWidth={2} strokeLinecap="round"/>
                      </svg>
                    </div>
          ...
          {/* Tools — Notion multi-select OR staticData fallback */}
          {tools.length > 0 && (
            <div>
              <p className="font-mono text-[10px] font-semibold tracking-widest text-brand-400/70 uppercase mb-3">
                Tools Used
...
function ServiceCard({ s, i, onClick }) {
  return (
    <button
      onClick={() => onClick(s)}
      className="flex flex-col justify-between card-glass p-7 text-left w-full group h-80"
    >
      <div>
        <div className="flex items-start justify-between mb-8">
          {s.logo
            ? <img src={s.logo} alt={s.title} className="w-11 h-11 rounded-xl object-contain" />
            : <div className="w-11 h-11 rounded-xl bg-brand-500/15 border border-brand-500/25 flex items-center justify-center text-xl font-mono text-brand-400 group-hover:scale-110 transition-transform duration-300">
                {s.icon}
              </div>
          }
          <span className="font-mono text-[11px] font-semibold text-blue-400/40">
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/notion-services`, {
      ...
      .catch(() => {
        ...
        setServices(SERVICES_DATA.map(s => ({ ...s, tools: s.tools || [], features: s.features || [], tagline: s.tagline || '' })))
      })
      ...
      <section className="py-16 border-b border-white/[0.05]">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          ...
          <p className="text-blue-200/60 text-base max-w-lg leading-relaxed">
            Click any service to learn more about what's included.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {loading
              ...
              : current.map((s, i) => (
                  <ServiceCard key={s.id || i} s={s} i={page * PER + i} onClick={setSelected} />
                ))
            }
          </div>
              ...
              {Array.from({ length: total }).map((_, i) => (
                <button
                  ...
                  style={{
                    width: i === page ? 28 : 8,
                    height: 8,
                    background: i === page ? '#2d8ef5' : 'rgba(45,142,245,0.2)'
                  }}
                />
              ))}
