...
const ALL_TAGS = ['All', ...Array.from(new Set(CASE_STUDIES.map(c => c.tag)))]

function CaseCard({ cs, i }) {
  return (
    <Link to={`/case-studies/${cs.id}`} className="card-glass p-7 flex flex-col gap-5 group block" style={{ transitionDelay: `${i*0.07}s` }}>
      ...
      <div className="flex flex-wrap gap-1.5">
        {cs.tools.map(t => (
          <span key={t} className="font-mono text-[10px] text-blue-300/50 bg-navy-700/60 border border-white/[0.06] px-2 py-0.5 rounded">{t}</span>
        ))}
      </div>
      <div className="flex items-center gap-1.5 text-brand-400 text-sm font-semibold mt-auto">
        <span>Read Case Study</span>
        <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24" className="group-hover:translate-x-1 transition-transform">
...
export default function CaseStudies() {
  ...
  const shown = filter === 'All' ? CASE_STUDIES : CASE_STUDIES.filter(c => c.tag === filter)

  // Track whether the navbar is visible (same logic as Navbar.jsx)
  // When navbar hides (slides up), filter bar moves to top-0
    ...
    <main className="pt-24">
      <section className="py-16 border-b border-white/[0.05]">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          ...
          <h1 className="section-title text-white mb-4">Case <span className="gradient-text-blue">Studies</span></h1>
          <p className="text-blue-200/60 text-base max-w-lg leading-relaxed">Real automation systems built for real clients. Click any project to read the full story.</p>
        </div>
      </section>

      <div
        className={`sticky z-40 border-b backdrop-blur-xl ${isDark ? 'border-white/[0.05] bg-navy-950/90' : 'border-brand-500/10 bg-blue-50/95'}`}
        style={{
          top: navVisible ? '68px' : '0px',
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {shown.map((cs, i) => <CaseCard key={cs.id} cs={cs} i={i} />)}
          </div>
        </div>
      </section>
      ...
      <section className="pb-20">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <Link to="/book"><button className="btn-primary px-8 py-4 text-base"><span>Book a Free Call →</span></button></Link>
          </div>
        </div>
      </section>
    </main>
